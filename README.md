...
1) vm `kali` отправляет пакет TCP с флагом FIN дважды. 
2) отсутствие ответа означает, что порт открыт|фильтруется.
 `nmap` помещает порт в это состояние, так как не может определить, открыт ли порт или отфильтрован, так как порт не дает ответа.
...
Xmas-сканирование не будет работать с ОС Windows, CISCO, IRIX, HP/UX и BSDI
...
- https://stuff.mit.edu/afs/athena/astaff/project/opssrc/nmap-3.00/docs/nmap_manpage-ru.html 